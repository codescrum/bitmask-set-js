...
class BitmaskSet {
  ...
  constructor(elements, options = {
    ...
    if (!Array.isArray(elements)) {
      throw Error('The set defined by all elements must be an array!');
    }
    
    // Check if elements has at least one element
    if (elements.length === 0) {
      throw Error('The set defined by all elements cannot be empty!');
    }
    ...
    this.max = BigInt('0b' + this.ones); // Max value
  }
