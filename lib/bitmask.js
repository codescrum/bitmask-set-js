...
class Bitmask {
  ...
  constructor(set, elements, options = {
    inherit_sort: true,
    sort: false
  }) {
    if (set === undefined || set === null) {
      throw Error('You need to specify a set for the bitmask to make sense.');
    } /// //// Initialize
    // Associate with set
    ...
    let sort = options.sort; // Check constructor arguments
    // Careful here: JSBI instances DO evaluate to being arrays.

    if (Array.isArray(elements) && !(typeof elements === "bigint")) {
      ...
      if (sort) {
        console.log('WARNING: Element sorting is not available if you build the bitmask from bits.');
      }

      const unpadded_string = bits.toString(2);

      if (unpadded_string.length > this.set.size) {
        throw Error("You are passing more bits than the set's size.");
      } // Computed representations (elements passed)
      ...
      if (string.length === 0) {
        throw Error('The string to build the bitmask from is empty!');
      }

      if (string.length < this.set.size) {
        throw Error("The string to build the bitmask from does not match the set's size (string is smaller in length).");
      }

      if (string.length > this.set.size) {
        throw Error("The string to build the bitmask from does not match the set's size (string is larger in length).");
      }

      if (!string.match('^[01]+$')) {
        throw Error("The string to build the bitmask from must only contain 0's or 1's.");
      } // Warn about sorting


      if (sort) {
        console.log('WARNING: Element sorting is not available if you build the bitmask from a string.');
      } // Computed representations (elements passed)
  ...
  and(other) {
    return this.set.bitmask(this.bits & other.bits);
  ...
  invert() {
    ...
    return this.set.bitmask(this.bits ^ this.set.max);
