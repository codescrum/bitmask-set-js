...
describe('BitmaskSet', function () {
  describe('#constructor', function () {
    describe('with invalid arguments', function () {
      it('throws an error if constructed with `undefined`', function () {
        assert.throws(function () {
          new BitmaskSet();
        }, Error, 'The set defined by all elements must be an array!');
        assert.throws(function () {
          new BitmaskSet(undefined);
        }, Error, 'The set defined by all elements must be an array!');
      });
      ...
      it('throws an error if constructed with an empty array `[]`', function () {
        assert.throws(function () {
          new BitmaskSet([]);
        }, Error, 'The set defined by all elements cannot be empty!');
      });
    });
    describe('with valid arguments', function () {
      it('correctly populates elements, size, ones, zeros, options', function () {
        ...
        assert(bitmask_set.elements.every(function (element) {
          return [1, 2, 3, 4].includes(element);
        }));
  ...
  describe('with valid sets', function () {
    ...
    before(function (done) {
      ...
      small_sorted_set = new BitmaskSet([0, 7, 4, 6, 3, 1, 2, 8, 8, 5], {
        sort: true
      });
      done();
    }); /// ///////////////////////////////////////////////////////////////////////////
